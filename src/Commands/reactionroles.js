...
module.exports = class ReactionRoles extends cmds {
    ...
    async c1(msg, c) {
        ...
        c1.on("collect", async m1 => {
            ...
            collectedmessage = await msg.guild.channels.cache.get(c).messages.fetch(m1.content) || await msg.guild.channels.cache.get(c).messages.cache.get(m1.content)
    ...
    async c2(msg, collectedmessage) {
        ...
        c2.on("collect", m2 => {
            if (m2.content.split(" ").length !== 2) return msg.s("Make sure you used the right format! Cancelling...")
            try {
                let emoji = require('discord.js').Util.parseEmoji(m2.content.split(" ")[0])
            ...
            try {
                m2.g.roles.cache.get(m2.content.split(" ")[1])
            } catch {
            ...
            msg.client.db.set(`${msg.guild.id}.rr.${collectedmessage.id}.${emoji.id ? emoji.id : emoji.name}`, m2.content.split(" ")[1])
            m2.delete()
